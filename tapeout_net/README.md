...
本目录是学习这个协议的笔记，以及在它上面实现 Conway's Game of Life 的完整设计。**全部为只读分析：没有连接钱包，没有发送任何交易，没有花费任何资金。**

## 1. 协议是怎么工作的

### 网表格式（整理自公开的前端代码 `assets/netlist-*.js`）
...
一个观察：13,401 个候选里目前有 4 个用了 REF。跨电路复用这个机制还处在早期，而它正是这个设计省 token 的关键。

原始数据不入库（`npm run survey` 重新生成，约一小时）。识别结果汇总如下，并注明出自哪个项目：
...
node scan_info.mjs && node classify2.mjs  # 重新整理链上电路（约 1 小时，受公共 RPC 限流）
```
...
|---|---|---|---|
| 官网画布导入 BLIF | 170 | 约 3,776（`out/life_grid_8.blif`，扁平） | 能在画布上看图和编辑；通用编译路径对纯 NAND 网表不占优 |
